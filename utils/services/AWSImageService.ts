...
function mapLocalImagesToProcessing(localImages: LocalImage[]): ProcessingImage[] {
  return localImages.map((i, index) => {
...
export const useAWSImageService = () => {
  ...
  const getImagesWithStatus = useCallback(async (): Promise<ProcessingImage[]> => {
    ...
    try {
      ...
      const processedImages = mapLocalImagesToProcessing(localImages);
